...
- **提交前必执行 `mise open-api`**：每次修改源码后（含 merge），重新生成 OpenAPI spec 和 SDK，确保生成的代码与源码一致。
- **CLAUDE.md 仅 develop 分支保留**：merge 到 `feat/*` 或 `main` 时不要带入 CLAUDE.md，通过 `git checkout upstream/main -- CLAUDE.md` 移除。
...
| 分支 | 用途 | CI 策略 |
|------|------|---------|
| `main` | 官方上游镜像 | 所有 Job 添加 `if: github.repository == 'immich-app/immich'`（repo guard），完全禁用 Actions |
| `develop` | 个人开发主分支 | CI 适配 `github.token` 替代 Push-O-Matic，Actions 正常构建，Docker 推送到 `ghcr.io/<owner>/immich-server:develop` |
| `feat/*` | 纯功能分支 | **不修改 `.github/`**，合并上游时 CI 文件直接从 develop 同步 |

**CLAUDE.md 仅存在于 develop 分支**，确保不在 `feat/*` 或 `main` 中出现。
...
```bash
mise open-api          # 重新生成 OpenAPI spec + TypeScript SDK + Dart SDK
```

然后检查生成的文件变更是否合理：

```bash
git diff --stat packages/sdk/ mobile/openapi/ open-api/
```

其他检查项：

```bash
git status | grep mise.lock   # mise.lock 不应该出现在暂存区
git status | grep CLAUDE.md   # CLAUDE.md 不应在 feat/* 或 main 中出现
```

## 上游同步

将 upstream/main 的更新合并到本地分支的标准化流程。
...
# 常见冲突：.github/workflows/test.yml
#   解决：combine repo guard + upstream 的新条件
mise open-api          # merge 后必须重新生成
# 检查所有 job 都有 repo guard
...
# 1. 解决翻译冲突：保留功能分支的翻译 key
# 2. 二进制 SDK 冲突（mobile/openapi/README.md）：
#    先暂存回退，然后重新生成
#    git checkout --ours mobile/openapi/README.md

# 3. 关键步骤：重新生成全部 SDK
mise open-api

# 4. 同步 CI 文件（避免后续 merge 到 develop 时 CI 冲突）
git checkout develop -- .github/

# 5. 确认 CLAUDE.md 不在变更中
git status | grep CLAUDE.md && git checkout upstream/main -- CLAUDE.md

git add -A && git commit
```

**原理**：feature 不自定义 CI，直接从 develop 同步，确保 merge 到 develop 时 CI 文件零差异，避免冲突。

### 同步到 develop

```bash
git checkout develop
git merge feat/xxx --no-commit     # 或 git merge upstream/main --no-commit
# CI 冲突时：git checkout --ours -- .github/
mise open-api          # merge 后必须重新生成
git add -A && git commit
```
